 ...
 * @author https://ethereum.stackexchange.com/questions/63908/address-checksum-solidity-implementation
 */
library AddressChecksumUtils {

  function _addressToString2(address x) internal pure returns (string memory) {
  ...
  function addressToString(address _addr) external pure returns(string memory str) {
  ...
  function _toChecksumString(
    ...
    for (uint256 i = 0; i < data.length; i++) {
      ...
      asciiBytes[2 * i] = bytes1(uint8(leftNibble + asciiOffset));

      // get the offset from nibble value to ascii character for right nibble.
      asciiOffset = _getAsciiOffset(rightNibble, rightCaps);

      // add the converted character to the byte array.
      asciiBytes[2 * i + 1] = bytes1(uint8(rightNibble + asciiOffset));
    }
      ...
      for (uint256 i; i < 40; i++) {
        ...
        if (i % 2 == 0) {
          ...
          accountAddressBytes[(i - 1) / 2] = (
            bytes1(uint8(16 * nibble + (b - asciiOffset))));
        }
      }
  ...
  function _toAsciiString(
    ...
    for (uint256 i = 0; i < data.length; i++) {
      ...
      asciiBytes[2 * i] = bytes1(uint8(leftNibble + (leftNibble < 10 ? 48 : 87)));
      asciiBytes[2 * i + 1] = bytes1(uint8(rightNibble + (rightNibble < 10 ? 48 : 87)));
    }
